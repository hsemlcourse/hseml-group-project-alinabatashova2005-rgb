...
rf_cv    = exp_df[exp_df["Модель"] == "Random Forest"]["CV F1"].values[0]
rf_t_cv  = exp_df[exp_df["Модель"] == "Random Forest (tuned)"]["CV F1"].values[0]
print(f"""
10. ВЫВОДЫ
    ...
    3. Протестировано 7 конфигураций: 5 базовых + 2 с подбором гиперпараметров (GridSearchCV).
       Результаты (CV F1 weighted):
         ...
         RF       {rf_cv:.4f}  - выигрывает на тесте
         RF tuned {rf_t_cv:.4f}  - GridSearch: {rf_tuned_params}
    4. Лучшая модель: {best_row['Модель']}
       Test F1={best_row['Test F1']}, CV={best_row['CV F1']}±{best_row['CV F1 std']}.
       CV=1.0000 - кластеры хорошо разделены в пространстве признаков на всех 5 фолдах.
    5. Почему Random Forest лучше LogReg на тесте:
       - CV одинаковый (0.9976), но RF даёт Test F1 на 1.3pp выше - устойчивее к новым городам
       ...
       - feature importances: яйца и зерновые - главные разделители сегментов
    6. Ограничения: RF сложнее объяснить на уровне отдельного предсказания, чем LR.
       При масштабировании стоит рассмотреть LightGBM - быстрее при сопоставимом качестве.
""")
